...
export class ItemsService {
	...
	async getItems() {
		const result = await getConnection()
			.createQueryBuilder()
			.select('i.title, i.image, i.rawimage, i.description, i.code, mi.id, mi.price')
	...
	async getPaytmQr(id: number) {
		...
		if (response.machine_id) {
			...
			const checksum = await PaytmChecksum.generateSignature(JSON.stringify(option.body), cred.key);
			option.head = {
				clientId: 'C112',
				version: 'v1',
				signature: checksum
	...
	async checkTransactionStatus(orderId: string) {
		...
		if (result) {
			...
			if (mid) {
				...
				const checksum = await PaytmChecksum.generateSignature(JSON.stringify(paytmParams.body), cred.key);
				paytmParams.head = {
					signature: checksum
				};
				const payurl = `https://securegw.paytm.in/v3/order/status`;
				...
				try {
					const response = await axios.post(payurl, JSON.stringify(paytmParams), {
						headers: header
					});
					//console.log(response.status, value.order_id);
					if (response.status === 200) {
						...
						if (
							response.data.body.resultInfo.resultStatus === TransactionStatus.TXN_SUCCESS ||
							response.data.body.resultInfo.resultStatus === TransactionStatus.TXN_FAILURE
						) {
							const orderdata = await Payment.findOne({ id: result.id });
							...
							socket.to(`MACHINE_${machine_id}`).emit('transaction-response', {
								orderdata: orderdata
							});
						}
	...
	async getOrderById(orderId: string) {
		await Payment.find({ order_id: orderId });
	}

	async cancelTransaction(orderId: string) {
		await Payment.update({ order_id: orderId }, { status: TransactionStatus.CANCEL });
	}
}
